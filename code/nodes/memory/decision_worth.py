...
to match future questions to it), summary (2-4 sentences capturing what was
learned this turn, in the same language as the user's question, written so it
stands alone), and keywords.
...
def decision_worth(state: State) -> dict:
    ...
    decider = get_memory_llm().with_structured_output(Decision)
