...
from repository.memory_repository import UserMemory, find_topic, render_index_for_router
...
def search_TopicIndex(state: State) -> dict:
    index = UserMemory(state["user_id"]).load_index()
    ...
    router = get_memory_llm().with_structured_output(RouteDecision)
