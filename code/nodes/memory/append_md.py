"""Memory WRITE — extend an existing topic. Diagram box: "Append/Compress topic .md"."""

from pydantic import BaseModel

from config.memory import COMPRESS_AT_CHARS
...
def _llm_compress(full_text: str) -> tuple[str, str]:
    compressor = get_memory_llm().with_structured_output(Compression)
...
def append_md(state: State) -> dict:
    ...
    result = UserMemory(state["user_id"]).append_topic(
        path_str=path,
        query=state["query"],
        answer=state["answer"],
    ...
    line = f"append_md: appended to '{path}', turn_count={result['turn_count']}"
    if result.get("keywords"):
        line += f", keywords={len(result['keywords'])}"
    if result["compressed"]:
        line += " (compressed summary + refreshed one_liner)"
    return {"trace": [line]}
